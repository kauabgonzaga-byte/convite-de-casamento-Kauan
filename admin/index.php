...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'save_gift') {
            ...
            $oldImage = '';
            try {
                update_json('gifts', function (array &$gifts) use ($id, $name, $price, $icon, $removeImage, $uploadedImage, &$oldImage): void {
      ...
      <div class="gift-manager">
        ...
        <div class="admin-table-wrap"><table><thead><tr><th>Foto</th><th>Presente</th><th>Valor</th><th>Status</th><th>Ações</th></tr></thead><tbody>
          <?php foreach ($gifts as $gift): ?>
            <?php $giftImage = gift_image_path($gift['image'] ?? null) !== null ? (string) $gift['image'] : ''; ?>
            <tr><td><?php if ($giftImage !== ''): ?><img class="gift-thumb" src="../<?= h($giftImage) ?>" alt="Foto de <?= h($gift['name'] ?? 'presente') ?>"><?php else: ?><span class="gift-thumb-placeholder" aria-label="Sem foto"><?= h($gift['icon'] ?? '✦') ?></span><?php endif; ?></td><td><strong><?= h($gift['name'] ?? '') ?></strong><?php if (!empty($gift['reserved_by'])): ?><small>por <?= h($gift['reserved_by']) ?></small><?php endif; ?></td><td><?= format_brl($gift['price'] ?? 0) ?></td><td><span class="status <?= ($gift['status'] ?? '') === 'reserved' ? 'reserved' : 'available' ?>"><?= ($gift['status'] ?? '') === 'reserved' ? 'Escolhido' : 'Disponível' ?></span></td><td class="actions">
              <a class="button-link" href="?edit=<?= rawurlencode((string) ($gift['id'] ?? '')) ?>#presentes">Editar</a><form method="post"><input type="hidden" name="csrf" value="<?= h(csrf_token()) ?>"><input type="hidden" name="action" value="release_gift"><input type="hidden" name="id" value="<?= h($gift['id'] ?? '') ?>"><button type="submit" class="button-link"<?= ($gift['status'] ?? '') === 'reserved' ? '' : ' disabled' ?>>Liberar</button></form>
